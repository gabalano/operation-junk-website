(() => {
  ...
  card.setAttribute('aria-label', 'Our past pickups');
  card.innerHTML = `
    <div class="oj-card-top"><div><span class="oj-card-kicker">WHAT WE DO</span><h2>Our past pickups</h2></div><div class="oj-job-index"><span data-job-current>01</span><span>/</span><span>${String(jobs.length).padStart(2, '0')}</span></div></div>
    <div class="oj-job-slider" role="region" aria-roledescription="carousel" aria-label="Operation Junk before and after pickup gallery">
      <div class="oj-job-slides">${slides}</div>
      <button class="oj-job-arrow oj-job-arrow--prev" type="button" data-job-prev aria-label="Previous pickup">‹</button>
      <button class="oj-job-arrow oj-job-arrow--next" type="button" data-job-next aria-label="Next pickup">›</button>
    </div>
    <div class="oj-job-footer"><a class="oj-job-cta" href="tel:${phone}">Need a job like this? Call <strong>(872) 222-5502</strong></a></div>`;
